...
if __name__ == "__main__":
    ...
    r.add_effect(effects.bounce.Bounce, r.Set_All())
    
    #id = r.add_effect('sine_wave', r.Set_Ring);
    #r.effect(id).color = ColorFade(start=r.Color_Red, finish=r.Color_Blue)


#    r.add_effect(effects.sine_wave.SineWave(r.Set_Half01, color=color.Color(1, 1, 0), period=2, direction=-1))
#    r.add_effect(Effect_SineWave(r.Set_Half23, color=Color(0, 1, 1), period=2, direction=1))

#    r.add_effect_object(effects.Bounce(r.Set_All, color=color.ColorFade(start=r.Color_Green, finish=r.Color_Violet)))

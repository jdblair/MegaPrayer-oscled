...
OSCServer::OSCServer(int port) : m_port(port)
{
    ...
    m_st->add_method("/led", "iiii", 
                    [this](lo_arg **argv, int)  {this->method_led(argv);});

}


int OSCServer::method_led(lo_arg **argv)
{
    cout << "/led (" << ++received << "): "
         << argv[0]->i << ", " 
         << argv[1]->i << ", " 

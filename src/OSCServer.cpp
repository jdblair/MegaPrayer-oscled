...
int OSCServer::bind(shared_ptr<IPlatformSerial> ser, int base, int len, bool reverse)
{
    ...
    for (auto i = 0; i < len; i++) {
        led_iface->leds.push_back(led_t(0,0,0));
    }

    // now start the update thread for this interface
    // this avoid a race condition that happens if the update_thread() is started
    // before the leds vector is initialized
    led_iface->start_update_thread();
